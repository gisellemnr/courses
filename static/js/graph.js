...
function Graph(semesters, ar) {
	...
	function position(y, shape, effect) { 
		...
		var left = (700 - (700 * (list.length - 1) / list.length)) / 2 + marginLeft;
		for (index in list) {
			if (shape) {
				list[index].animate({
					cx: x,
	...
	function createShape(course) {
		...
		shape.attr({
			fill: shape.color,
			stroke: shape.color,
			"stroke-width": shape.area == "placeholder"? 0 : 2,
			cursor: "move",
			"fill-opacity": 0.7
		}).toFront();
		label.attr({
		...
		$.each(list, function (i, shape) {
			...
			for (var i = connections.length; i--;) {
				if (connections[i].from.id == shape.id || connections[i].to.id == shape.id) {
					...
					connections[i].arrow.attr({stroke: shape.attrs.fill, fill: shape.attrs.fill});
					connections[i].arrow.animate({
						opacity: 1
					}, 500);
	...
	function initGraph(semesters) {
		...
		var remove = r.circle(650 + marginLeft, 25, 20).attr({
			fill: '#555',
			"fill-opacity": .3,
			"stroke-width": 0,
			opacity: 0
		});
		var removelab = r.text(650 + marginLeft, 25, 'X').attr({
			stroke: "none",
			fill: "#000",
			opacity: 0
		}).toFront();
		var removetxt = r.text(650 + marginLeft, 25 + 40, 'Drag here to remove').attr({
			stroke: "none",
			fill: "#fff"
		});
		...
		r.text(10, 362.5, 'sophomore').attr(labelattr);
		r.text(10, 512.5, 'freshman').attr(labelattr);
